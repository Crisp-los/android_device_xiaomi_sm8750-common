...
static const fingerprint_hal_t kModules[] = {
        {"goodix_us"},  {"qcom_us"},     {"fortsense"}, {"fpc"},    {"fpc_fod"},
        {"goodix"},     {"goodix:gf_fingerprint"},      {"goodix_fod"},
        {"goodix_fod6"}, {"silead"},     {"syna"},
};

bool addUniqueModule(std::vector<std::string>* modules, const std::string& module) {
    if (module.empty()) {
        return false;
    }
...
std::vector<std::string> getFingerprintHalProbeOrder() {
    std::vector<std::string> modules;

    addUniqueModule(&modules, ::android::base::GetProperty("persist.vendor.sys.fp.vendor", ""));
    addUniqueModule(&modules,
                    ::android::base::GetProperty("persist.vendor.sys.fp.bigdata.type", ""));
...
Fingerprint::Fingerprint(std::shared_ptr<FingerprintConfig> config) : mConfig(std::move(config)) {
    ...
    if (mDevice) {
        ...
        auto modules = getFingerprintHalProbeOrder();
        ALOGI("fingerprint HAL probe order: %s", ::android::base::Join(modules, ", ").c_str());
    ...
    if (sensorTypeProp == "udfps" || sensorTypeProp == "udfps_optical") {
        ...
        if (!mDevice) {
            ALOGE("Skipping UdfpsHandler init because no fingerprint HAL is open");
        } else {
...
ndk::ScopedAStatus Fingerprint::createSession(int32_t /*sensorId*/, int32_t userId,
    ...
    if (!mDevice) {
        ALOGE("Creating unavailable fingerprint session: no valid fingerprint HAL device");
    }
