...
void notifyHwUnavailable(const std::shared_ptr<ISessionCallback>& cb, const char* operation) {
    ALOGE("%s failed: no valid fingerprint HAL device", operation);
    if (cb) {
        cb->onError(Error::HW_UNAVAILABLE, 0 /* vendorCode */);
    }
}
...
Session::Session(fingerprint_device_t* device, UdfpsHandler* udfpsHandler, int userId,
    ...
    if (mDevice) {
        ...
        ALOGE("Session created without a valid fingerprint HAL device");
    }
}

ndk::ScopedAStatus Session::generateChallenge() {
    if (!mDevice) {
        ALOGE("%s failed: no valid fingerprint HAL device", __func__);
        if (mCb) {
            mCb->onChallengeGenerated(0);
        }
...
ndk::ScopedAStatus Session::enroll(const HardwareAuthToken& hat,
    ...
    *out = SharedRefBase::make<CancellationSignal>(this);
    ...
    if (!mDevice) {
        notifyHwUnavailable(mCb, __func__);
        return ndk::ScopedAStatus::ok();
    }
...
ndk::ScopedAStatus Session::enumerateEnrollments() {
    if (!mDevice) {
        ALOGE("enumerateEnrollments: no valid fingerprint HAL device");
        mCb->onEnrollmentsEnumerated({});
        return ndk::ScopedAStatus::ok();
    }
...
ndk::ScopedAStatus Session::removeEnrollments(const std::vector<int32_t>& enrollmentIds) {
    ...
    if (!mDevice) {
        ALOGE("removeEnrollments: no valid fingerprint HAL device");
    ...
    if (error) {
        ...
        mCb->onError(Error::UNABLE_TO_REMOVE, error);
    }
#endif
    return ndk::ScopedAStatus::ok();
}
...
ndk::ScopedAStatus Session::getAuthenticatorId() {
    if (!mDevice) {
        ALOGE("getAuthenticatorId: no valid fingerprint HAL device");
...
ndk::ScopedAStatus Session::invalidateAuthenticatorId() {
    if (!mDevice) {
        ALOGE("invalidateAuthenticatorId: no valid fingerprint HAL device");
...
ndk::ScopedAStatus Session::resetLockout(const HardwareAuthToken& hat) {
    ...
    if (mDevice) {
        ...
        if (resetResult != 0) {
            ...
            mCb->onError(Error::UNABLE_TO_PROCESS, resetResult);
        }
    } else {
        ALOGE("resetLockout: no valid fingerprint HAL device");
    }
#else
    (void)hat;
#endif
...
ndk::ScopedAStatus Session::cancel() {
    ...
    ALOGE("cancel failed: %d", ret);
    mCb->onError(Error::UNABLE_TO_PROCESS, ret);
    return ndk::ScopedAStatus::ok();
}
